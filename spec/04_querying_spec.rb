puts describe 'querying' do
  ...
  puts it 'selects all of the books titles and years in the first series and orders them chronologically' do
...
puts   it 'returns the name and motto of the character with the longest motto' do
  ...
  puts it 'determines the most prolific species of characters and return its value and count' do
    expect(@db.execute(select_value_and_count_of_most_prolific_species)).to eq([["human", 4]])
  end

  puts it "selects the authors names and their series' subgenres" do
    expect(@db.execute(select_name_and_series_subgenres_of_authors)).to eq([["George R. R. Martin", "medieval"], ["Second Author", "space opera"]])
  end

  puts it 'selects the series title with the most characters that are the species "human"' do
    expect(@db.execute(select_series_title_with_most_human_characters)).to eq([["A Song of Ice and Fire"]])
  end

  puts it 'selects all of the character names and their number of books they have appeared in, in descending order' do
