describe 'querying' do
  ...
  it 'returns the name and motto of the character with the longest motto' do
    #I changed this test so that the correct result matches the data I created; my characters have much longer mottos
    ...
    expect(@db.execute(select_name_and_motto_of_char_with_longest_motto)).to eq([["Valentine", "Welcome to the human race. Nobody controls his own life, Ender. The best you can do is choose
      to fill the roles given you by good people, by people who love you."]])
  end

  it 'determines the most prolific species of characters and return its value and count' do
    #I changed this test so that the correct result matches the data I created
    #expect(@db.execute(select_value_and_count_of_most_prolific_species)).to eq([["human", 4]])
    ##two of the characters I added are human
