...
def select_name_and_series_subgenres_of_authors
  "SELECT authors.name as author_name, subgenres.name as subgenre_name FROM authors JOIN series ON authors.id = series.author_id JOIN subgenres ON series.subgenre_id = subgenres.id;"
end

def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN characters ON series.id = characters.series_id GROUP BY series.title ORDER BY count(characters.species) LIMIT 1;"
end
