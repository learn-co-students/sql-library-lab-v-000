...
def select_series_title_with_most_human_characters
  "SELECT Series.title FROM series JOIN books ON Series.id = Books.series_id JOIN character_books ON Character_books.book_id = Books.id JOIN characters ON Character_books.character_id = Characters.id WHERE Characters.species = 'human' GROUP BY Series.title ORDER BY COUNT(Characters.species) DESC LIMIT 1;"
end
