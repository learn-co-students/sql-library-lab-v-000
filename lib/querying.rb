...
def select_name_and_motto_of_char_with_longest_motto
  ...
  WHERE length(characters.motto) > 25;"
  #LIMIT 1;
end

def select_value_and_count_of_most_prolific_species
  "SELECT characters.species, count(characters.species) as value_occurrence FROM characters
...
def select_name_and_series_subgenres_of_authors
  ...
  JOIN subgenres WHERE authors.id = subgenres.id;"
end

def select_series_title_with_most_human_characters
  "SELECT series.title FROM series
  JOIN characters WHERE characters.species = 'human'
