...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series INNER JOIN characters on series.id = characters.series_id GROUP BY series.title HAVING COUNT(characters.species = 'human') Order BY COUNT(characters.species = 'human') DESC LIMIT 1;"
end
