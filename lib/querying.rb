...
def select_name_and_series_subgenres_of_authors
  ...
  INNER JOIN series ON authors.id = series.author_id AND series.subgenre_id = subgenres.id"
end

def select_series_title_with_most_human_characters
  "SELECT series.title FROM series
  INNER JOIN characters ON characters.series_id = series.id
  GROUP BY characters.series_id ORDER BY COUNT(characters.series_id) DESC LIMIT 1"
end
