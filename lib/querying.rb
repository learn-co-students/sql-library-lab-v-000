...
def select_value_and_count_of_most_prolific_species
  ...
  species ORDER BY COUNT(species) DESC LIMIT 1;"
end
...
def select_series_title_with_most_human_characters
  ...
  characters.series_id = series.id WHERE characters.species = 'human' GROUP BY series.title 
  ORDER BY COUNT(characters.species) DESC LIMIT 1;" 
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(character_books.id) FROM 
  characters INNER JOIN character_books 
