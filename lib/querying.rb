...
def select_value_and_count_of_most_prolific_species
  "SELECT characters.species, COUNT(characters.species) FROM characters INNER JOIN character_books ON characters.id = character_books.id GROUP BY characters.species ORDER BY COUNT(characters.species) DESC LIMIT 1;"
end
...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series INNER JOIN characters ON series.id = characters.series_id GROUP BY series.title HAVING COUNT(characters.species = 'human')>1 LIMIT 1;"
end
