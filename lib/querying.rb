...
def select_series_title_with_most_human_characters
  "SELECT series.title  FROM series GROUP BY series.title ORDER BY series.title LIMIT 1"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(*) AS book_char FROM character_books JOIN characters ON characters.id = character_books.character_id GROUP BY characters.name ORDER BY book_char DESC"
end
