...
def select_series_title_with_most_human_characters
  "SELECT Series.title FROM Series INNER JOIN Characters ON Series.id = Characters.series_id GROUP BY Series.id ORDER BY Count(*) DESC LIMIT 1;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT Characters.name, COUNT(1) FROM Characters INNER JOIN Character_Books ON Characters.id = Character_Books.character_id GROUP BY Characters.name ORDER BY COUNT(1) DESC;"
end
