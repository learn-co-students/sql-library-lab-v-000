...
def select_series_title_with_most_human_characters
  "SELECT title FROM series JOIN characters ON series.id = characters.series_id GROUP BY title, species HAVING species = 'human' ORDER BY COUNT(species) DESC LIMIT 1;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(book_id) FROM character_books JOIN characters ON character_id = characters.id GROUP BY character_id ORDER BY COUNT(book_id) DESC, characters.name;"
end
