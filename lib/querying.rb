...
def select_value_and_count_of_most_prolific_species
  "SELECT species, COUNT(*) AS most_prolific FROM characters GROUP BY species ORDER BY COUNT(*) DESC LIMIT 1"
end
...
def select_series_title_with_most_human_characters
  "SELECT title FROM series INNER JOIN characters ON characters.series_id = series.id WHERE species = 'human' GROUP BY title ORDER BY COUNT(species) DESC LIMIT 1"
end
